...
ov_status_e ov_node_list_get_shape_by_index(ov_output_node_list_t* port_list, size_t idx, ov_shape_t* tensor_shape) {
    if (!nodes || idx >= port_list->size || !tensor_shape) {
        return ov_status_e::INVALID_C_PARAM;
    }

    try {
        auto shape = port_list->output_nodes[idx].object->get_shape();
        ov_shape_init(tensor_shape, shape.size());
        std::copy_n(shape.begin(), shape.size(), tensor_shape->dims);
    }
    CATCH_OV_EXCEPTIONS
...
ov_status_e ov_node_list_get_any_name_by_index(ov_output_node_list_t* port_list, size_t idx, char** tensor_name) {
    if (!nodes || !tensor_name || idx >= port_list->size) {
        return ov_status_e::INVALID_C_PARAM;
    }
...
ov_status_e ov_node_list_get_partial_shape_by_index(ov_output_node_list_t* port_list,
                                                    size_t idx,
                                                    ov_partial_shape_t** partial_shape) {
    if (!nodes || idx >= port_list->size || !partial_shape) {
        return ov_status_e::INVALID_C_PARAM;
    }

    try {
        std::unique_ptr<ov_partial_shape_t> _partial_shape(new ov_partial_shape_t);
        auto shape = port_list->output_nodes[idx].object->get_partial_shape();
...
ov_status_e ov_node_list_get_element_type_by_index(ov_output_node_list_t* port_list,
                                                   size_t idx,
                                                   ov_element_type_e* tensor_type) {
    if (!nodes || idx >= port_list->size) {
        return ov_status_e::INVALID_C_PARAM;
    }
