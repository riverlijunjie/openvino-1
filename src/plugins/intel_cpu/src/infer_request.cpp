...
SyncInferRequest::SyncInferRequest(std::shared_ptr<const CompiledModel> compiled_model)
    ...
    for (const auto& in : orig_model->inputs()) {
        ...
        std::cout << "orig input port: name = " << port_name << ", precision = " << in.get_element_type()
                  << ", shape = " << in.get_partial_shape().to_string() << std::endl;
    ...
    for (const auto& out : orig_model->outputs()) {
        ...
        std::cout << "orig output port: name = " << port_name << ", precision = " << out.get_element_type()
                  << ", shape = " << out.get_partial_shape().to_string() << std::endl;
    ...
    std::cout << std::endl;
    for (auto& it : _port_precision_changed) {
        std::cout << "    precision change: " << it.first << " - " << it.second << std::endl;
    }
    std::cout << std::endl;
}
...
std::string SyncInferRequest::query_port_name(const ov::Output<const ov::Node>& port) const {
    ...
    // WA: In some case, input name has been changed after transformation(SimpleIfTest), but user maybe pass
...
ov::Tensor SyncInferRequest::get_tensor(const ov::Output<const ov::Node>& _port) const {
    ...
    std::cout << "get_tensor: name = " << port_name << std::endl;
    std::cout << "      argument port precision = " << _port.get_element_type()
              << ", shape = " << _port.get_partial_shape().to_string() << std::endl;
    std::cout << "      compiled port precision = " << port.get_element_type()
              << ", shape = " << port.get_partial_shape().to_string() << std::endl;
    std::cout << "        origin port precision = " << _orig_ports_map[port_name].get_element_type()
              << ", shape = " << _orig_ports_map[port_name].get_partial_shape().to_string() << std::endl;
    std::cout << "                is compiled port = " << is_compiled_port << std::endl;
    ...
    if (_aux_tensors.find(port_name) == _aux_tensors.end()) {
        ...
        std::cout << "        create external tensor, precision " << _aux_tensors[port_name].get_element_type()
                  << ", shape = " << _aux_tensors[port_name].get_shape().to_string() << std::endl;
    } else {
        std::cout << "        get external tensor, precision " << _aux_tensors[port_name].get_element_type()
                  << ", shape = " << _aux_tensors[port_name].get_shape().to_string() << std::endl;
    }
...
void SyncInferRequest::set_tensor(const ov::Output<const ov::Node>& _port, const ov::Tensor& _tensor) {
    ...
    std::cout << "set port: name = " << name << std::endl;
    std::cout << "      argument port precision = " << _port.get_element_type()
              << ", shape = " << _port.get_partial_shape().to_string() << std::endl;
    std::cout << "      compiled port precision = " << port.get_element_type()
              << ", shape = " << port.get_partial_shape().to_string() << std::endl;
    std::cout << "        origin port precision = " << _orig_ports_map[name].get_element_type()
              << ", shape = " << _orig_ports_map[name].get_partial_shape().to_string() << std::endl;
    std::cout << "        tensor precision = " << _tensor.get_element_type()
              << ", shape = " << _tensor.get_shape().to_string() << std::endl;
    std::cout << "         is_compiled_port = " << is_compiled_port << std::endl;
    std::cout << "         is_imported_model = " << is_imported_model << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "      update compiled tensor shape: " << tensor.get_shape().to_string() << std::endl;
...
void SyncInferRequest::PushInputData() {
    for (auto input : get_inputs()) {
        ...
        if (_aux_tensors.find(input_name) != _aux_tensors.end()) {
            ...
            std::cout << "PushInputData: covert data " << aux_tensor.get_element_type() << " --> "
                      << tensor.get_element_type() << std::endl;
            cpu_convert(srcData,
                        dstData,
                        InferenceEngine::details::convertPrecision(aux_tensor.get_element_type()),
