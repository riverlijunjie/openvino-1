...
public:
    explicit MoE3GemmSwigluPrefillGather(bool use_grouped_gemm = false)
        : KernelGenerator("moe_gather_ref", "prefill_gather"), m_use_grouped_gemm(use_grouped_gemm) {}
...
public:
    explicit MoE3GemmSwigluPrefillSwiglu(bool use_grouped_gemm = false)
        : KernelGenerator("moe_3gemm_swiglu_fuse", "prefill_swiglu"), m_use_grouped_gemm(use_grouped_gemm) {}
...
public:
    explicit MoE3GemmSwigluPrefillScatterReduce(bool use_grouped_gemm = false)
        : KernelGenerator("moe_scatter_reduction_opt", "moe_scatter_reduction_ref"), m_use_grouped_gemm(use_grouped_gemm) {}
...
public:
    ...
    moe_3gemm_swiglu_opt_impl(const program_node& node, const RuntimeParams& params) : moe_3gemm_swiglu_opt_impl() {
        ...
        GPU_DEBUG_TRACE_DETAIL << "[DEBUG] moe_3gemm_swiglu_opt_impl(): use_grouped_gemm_prefill="
                               << use_grouped_gemm_prefill << std::endl;
...
#    endif
    ...
    grouped_onednn_kernel& get_grouped_kernel(int total_tokens,
                                              typed_primitive_inst<moe_3gemm_fused_compressed>& instance) {
        ...
        auto a_dt = convert_data_type(
            instance.input_memory_ptr(static_cast<size_t>(MOE3GemmInputIndex::HIDDEN_STATES))->get_layout().data_type);
        auto gw_dt = convert_data_type(
            instance.input_memory_ptr(static_cast<size_t>(MOE3GemmInputIndex::WEIGHT_0))->get_layout().data_type);
        auto uw_dt = convert_data_type(
            instance.input_memory_ptr(static_cast<size_t>(MOE3GemmInputIndex::WEIGHT_1))->get_layout().data_type);
        auto dw_dt = convert_data_type(
            instance.input_memory_ptr(static_cast<size_t>(MOE3GemmInputIndex::WEIGHT_2))->get_layout().data_type);
        ...
        auto make_pd = [&](int K, int N, int group_size, dnnl::memory::data_type w_dt) {
            ...
            auto src_md = dnnl::memory::desc::grouped(
                dnnl::memory::dims{total_tokens, K}, a_dt, 0, num_experts);
            auto dst_md = dnnl::memory::desc::grouped(
                dnnl::memory::dims{total_tokens, N}, a_dt, 0, num_experts);
            // Weight: logical [E, K, N], physical layout acb -> stored as [E, N, K]
            auto w_md = dnnl::memory::desc(
                dnnl::memory::dims{num_experts, K, N}, w_dt, dnnl::memory::format_tag::acb);
        ...
        gk->gate_scale_md = make_quant_md(num_experts, K_gu, _gate_up_group_size, N_gu,
                                          dnnl::memory::data_type::f16);
        if (has_zp) gk->gate_zp_md = make_quant_md(num_experts, K_gu, _gate_up_group_size, N_gu, gw_dt);

        gk->up_pd     = make_pd(K_gu, N_gu, _gate_up_group_size, uw_dt);
        gk->up_prim   = dnnl::matmul(gk->up_pd);
        gk->up_scale_md = gk->gate_scale_md;
        if (has_zp) gk->up_zp_md = gk->gate_zp_md;

        gk->down_pd   = make_pd(K_d, N_d, _down_group_size, dw_dt);
        gk->down_prim = dnnl::matmul(gk->down_pd);
        gk->down_scale_md = make_quant_md(num_experts, K_d, _down_group_size, N_d,
                                          dnnl::memory::data_type::f16);
        if (has_zp) gk->down_zp_md = make_quant_md(num_experts, K_d, _down_group_size, N_d, dw_dt);
    ...
    cldnn::event::ptr exec_prefill_grouped_gemm(const std::vector<cldnn::event::ptr>& events,
        ...
        OV_ITT_SCOPED_TASK(ov::intel_gpu::itt::domains::intel_gpu_plugin,
                           openvino::itt::handle("moe_3gemm_swiglu_opt_impl::exec_prefill_grouped_gemm"));
        ...
        auto [hidden_states_mem_ptr, hidden_states_layout] =
            get_input_info(instance, static_cast<size_t>(MOE3GemmInputIndex::HIDDEN_STATES));
        auto token_num = get_seq_len(hidden_states_layout);
        auto final_hidden_states_mem_ptr = instance.output_memory_ptr(0);
        auto batch_mem_ptr   = scratch.topk_id;
        auto routing_mem_ptr = scratch.topk_weights;
        ...
        GPU_DEBUG_TRACE_DETAIL << "\nexec_prefill_grouped_gemm: token_num=" << token_num
                               << ", total_gathered_tokens=" << total_gathered_tokens
                               << ", num_actually_used_experts=" << num_actually_used_experts << std::endl;

        // Upload scratch metadata for the scatter_reduce and gather kernels
        intermediates_memories[MOE_INTERNAL_BUFFER_TOKEN_IDX_PER_EXPERT]
            ->copy_from(stream, tokens_per_expert_cpu.data(), 0, 0,
                        tokens_per_expert_cpu.size() * sizeof(int32_t), true);
        ...
        // = exclusive end of expert k = grouped_offsets_cpu[k].
        {
            ...
            intermediates_memories[MOE_INTERNAL_BUFFER_TOKEN_START_OFFSET_PER_EXPERT]
                ->copy_from(stream, expert_start_offsets_per_id.data(), 0, 0,
                            expert_start_offsets_per_id.size() * sizeof(int32_t), true);
        }
        intermediates_memories[MOE_INTERNAL_BUFFER_ACTIVATED_EXPERT_IDS]
            ->copy_from(stream, experts_id_cpu.data(), 0, 0,
                        static_cast<size_t>(num_actually_used_experts) * sizeof(int32_t), true);
        intermediates_memories[MOE_INTERNAL_BUFFER_TOKEN_LEN_PER_ACTIVATED_EXPERT]
            ->copy_from(stream, tokens_lens_per_expert_cpu.data(), 0, 0,
                        static_cast<size_t>(num_actually_used_experts) * sizeof(int32_t), true);
        intermediates_memories[MOE_INTERNAL_BUFFER_ACTUAL_USED_EXPERT_NUM]
        ...
        intermediates_memories[MOE_INTERNAL_BUFFER_GROUPED_OFFSETS]
            ->copy_from(stream, grouped_offsets_cpu.data(), 0, 0,
                        grouped_offsets_cpu.size() * sizeof(int32_t), true);
        ...
        // ----------------------------------------------------------------
        {
            ...
            auto block_size  = get_vec_size(*instance.get_impl_params());
            auto [local_threads_count, batches_per_thread, unaligned_elements] =
                calc_thread_count(const_cast<RuntimeParams&>(*instance.get_impl_params()),
                                  block_size, hidden_size);

            ret_event = execute_stage(
                {ret_event},
                instance,
        ...
        auto make_grouped_mem = [&](const dnnl::memory::desc& md, void* buf_ptr) {
            return dnnl::ocl_interop::make_memory(md,
                                                  ...
                                                  {reinterpret_cast<uint8_t*>(buf_ptr),
                                                   reinterpret_cast<uint8_t*>(offsets_ptr)});
        };
        ...
        // gate GEMM: [total, hidden] * W_gate[E,hidden,inter] -> [total, inter]
        {
            ...
            auto w_mem     = dnnl::ocl_interop::make_memory(gk.gate_pd.weights_desc(), onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
                                 scratch.moe_fusion_wei_addr.weight[0]->buffer_ptr());
            auto scale_mem = dnnl::ocl_interop::make_memory(gk.gate_scale_md, onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
            ...
            if (gk.has_zp) {
                args.insert({DNNL_ARG_ATTR_ZERO_POINTS | DNNL_ARG_WEIGHTS,
                             dnnl::ocl_interop::make_memory(gk.gate_zp_md, onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
        ...
        // up GEMM: [total, hidden] * W_up[E,hidden,inter] -> [total, inter]
        {
            ...
            auto w_mem     = dnnl::ocl_interop::make_memory(gk.up_pd.weights_desc(), onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
                                 scratch.moe_fusion_wei_addr.weight[1]->buffer_ptr());
            auto scale_mem = dnnl::ocl_interop::make_memory(gk.up_scale_md, onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
            ...
            if (gk.has_zp) {
                args.insert({DNNL_ARG_ATTR_ZERO_POINTS | DNNL_ARG_WEIGHTS,
                             dnnl::ocl_interop::make_memory(gk.up_zp_md, onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
        ...
        // passing ret_event (from gather) as dependency ensures ordering within the queue.
        {
            const size_t subgroup_size =
                instance.get_impl_params()->get_device_info().arch >= gpu_arch::xe2 ? 32 : 16;

            ret_event = execute_stage(
                {ret_event},
                instance,
                *grouped_gemm_prefill_swiglu,
                {intermediates_memories[MOE_INTERNAL_BUFFER_UP_OUTPUT],
                 intermediates_memories[MOE_INTERNAL_BUFFER_GATE_OUTPUT]},
                {intermediates_memories[MOE_INTERNAL_BUFFER_GATE_OUTPUT]},
                {static_cast<size_t>(_intermediate_size),
                 static_cast<size_t>(total_gathered_tokens), 1},
                {subgroup_size, 1, 1});
        }
        ...
        // down GEMM: [total, inter] * W_down[E,inter,hidden] -> [total, hidden]
        {
            ...
            auto w_mem     = dnnl::ocl_interop::make_memory(gk.down_pd.weights_desc(), onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
                                 scratch.moe_fusion_wei_addr.weight[2]->buffer_ptr());
            auto scale_mem = dnnl::ocl_interop::make_memory(gk.down_scale_md, onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
            ...
            if (gk.has_zp) {
                args.insert({DNNL_ARG_ATTR_ZERO_POINTS | DNNL_ARG_WEIGHTS,
                             dnnl::ocl_interop::make_memory(gk.down_zp_md, onednn_engine,
                                 dnnl::ocl_interop::memory_kind::usm,
    ...
    cldnn::event::ptr execute(const std::vector<cldnn::event::ptr>& events, cldnn::primitive_inst& ins) override {
        ...
        std::cout << "\nMoE3GemmFusedCompressed exec(): token_num=" << token_num << ", max_topk=" << static_cast<int>(config.top_k)
                  << ", use_micro_gemm_prefill=" << use_micro_gemm_prefill << ", use_grouped_gemm_prefill=" << use_grouped_gemm_prefill << std::endl;
