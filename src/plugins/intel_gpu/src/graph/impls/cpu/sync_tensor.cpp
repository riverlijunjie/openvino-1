...
#include "/home/openvino-ci-81/river/openvino/src/plugins/intel_gpu/src/runtime/ocl/ocl_stream.hpp"
...
public:
    ...
    cl_mem map_remote_mem(cl_context context, cl_mem clbuf, size_t size) {
        ...
        std::cout << "derive_map_remote_mem host time: " << elapsed_1.count() << " ms" << std::endl << std::endl;

        return extMemBuffer;
    }

cl_mem map_remote_mem(cl_context context, uint64_t nativeHandle, size_t size) {
        cl_int err;
        const auto start_1 = std::chrono::high_resolution_clock::now();
        // Create extMemBuffer of type cl_mem from fd.
        cl_mem_properties extMemProperties[] = {(cl_mem_properties)CL_EXTERNAL_MEMORY_HANDLE_DMA_BUF_KHR,
        ...
        const auto end_1 = std::chrono::high_resolution_clock::now();
        const std::chrono::duration<double, std::milli> elapsed_1 = end_1 - start_1;
        std::cout << "map_remote_mem host time: " << elapsed_1.count() << " ms" << std::endl << std::endl;
    ...
    void remote_write(cldnn::stream& stream, cl_mem src, cl_mem dst, size_t elemCount) {
        ...
        const char write_kernel_code[] = R"(
            kernel void write_to_remote(const global char *src, global char *dst)
            {
        ...
        const auto end_1 = std::chrono::high_resolution_clock::now();
        const std::chrono::duration<double, std::milli> elapsed_1 = end_1 - start_1;
        std::cout << "ocl p2p host time: " << elapsed_1.count() << " ms, size = " << elemCount * 4.0 / 1024
                  << "KB, bandwidth = " << elemCount * 1000.0 / 1024 / 1024 / 1024 / elapsed_1.count() << " GB / s "
                  << std::endl
                  << std::endl;
    }
...
inline kernel_data_type element_type_to_kernel_data_type(ov::element::Type_t element_type) {
    ...
    default:
        printf("Error: unsupported element type for kernel adder\n");
        break;
    }
    return kernel_data_type::e_type_int8;
}
...
public:
    ...
    cl_kernel get_or_create_kernel_if_possible(cldnn::stream& stream, kernel_data_type type) {
        if (type == kernel_data_type::e_type_fp16) {
            ...
            kernel void tensor_add_func_fp16(const global cl_half *src, global cl_half *dst)
            {
                const int id = get_global_id(0);
                dst[id] += src[id];
    ...
    void tensor_add(cldnn::stream& stream, cl_mem src, cl_mem dst, size_t element_count, kernel_data_type data_type) {
        cl_int err;
        auto& ocl_stream = downcast<ocl::ocl_stream>(stream);
        ...
        const auto end_1 = std::chrono::high_resolution_clock::now();
        const std::chrono::duration<double, std::milli> elapsed_1 = end_1 - start_1;
        std::cout << "tensor add host time: " << elapsed_1.count() << " ms, size = " << element_count << std::endl
                  << std::endl;
    }
        ...
        while (true) {
            ...
            for (int idx = 0; idx < static_cast<int>(w_size); idx++) {
                if (idx != w_rank && wait_list[idx] > 0 && sub_mem_mgr->_memorys_table[id][idx].flag) {
                    ...
                    auto dst_cl_buf = p2p_helper.map_remote_mem(local_context, dst_cl_buf_remote, dst_mem->size());
                    p2p_helper.remote_write(stream, src_cl_buf, dst_cl_buf, dst_mem->size());
                    uint64_t remote_fd = p2p_helper.derive_handle(dst_cl_buf);
                    sub_mem_mgr->_memorys_table[id][idx].remote_fd[w_rank] = remote_fd;
                    ...
                    // TODO: WA map back this cl_mem
                    // dst_cl_buf = p2p_helper.map_remote_mem(remote_context, dst_cl_buf, dst_mem->size());
                    //p2p_helper.destory_remote_mem(dst_cl_buf);
                }
        ...
        if (instance.get_impl_params()->need_add) {
            ...
            */
            ...
            for (size_t idx = 0; idx < w_size; idx++) {
                ...
                //                       ->get_buffer()
                //                       .get();
                auto fd = sub_mem_mgr->_memorys_table[id][w_rank].remote_fd[idx];
                auto src_cl_buf = p2p_helper.map_remote_mem(local_context, fd, dst_mem->size());
