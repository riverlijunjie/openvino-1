...
struct rms_impl : typed_primitive_impl_ocl<rms> {
    ...
    static void set_padding(const kernel_impl_params& impl_param, kernel_selector::rms_params* rms_params) {
        ...
        if (rms_params->dynamic_padding && rms_params->slice_stride == 0) {
            ...
            // std::cout << "layout: " << input_layout.to_string() << std::endl;
            // std::cout << "dynamic_pad_mask: " << dynamic_pad_mask.to_string() << std::endl;
            // std::cout << "lower_pads: ";
            // for (size_t i = 0; i < lower_pads.size(); i++)
            //     std::cout << lower_pads[i];
            // std::cout << std::endl;
            // std::cout << "upper_pads: ";
            // for (size_t i = 0; i < upper_pads.size(); i++)
            //     std::cout << upper_pads[i];
